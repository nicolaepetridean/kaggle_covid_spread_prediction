...
class ReccurentTrainer(BaseTrainer):
    ...
    def evaluate(self, testData):
        # TODO: Refactor
        evalVals = {key:[] for key,_  in self.evalLossDict.items()}
        obsData   = testData[0]
        labelData = testData[1]

        for obs, label in zip(obsData,labelData):
            res  = self.model(obs.to(self.cfg.toDevice))
            for key, loss in self.evalLossDict.items():
                lossVal  = float(loss(res, label.to(self.cfg.toDevice)).cpu().detach())
                evalVals[key].append(lossVal)
